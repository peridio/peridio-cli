...
// use crate::config::config_v1::ProfileNameV1;
use crate::config::config_v1::ProfileV1;
use crate::config::config_v1::ProfilesV1;
...
impl Default for ProfilesV2 {
    fn default() -> ProfilesV2 {
        ProfilesV2(HashMap::new())
    }
}
...
impl TryFrom<ConfigV1> for ProfilesV2 {
    ...
    fn try_from(config_v1: ConfigV1) -> Result<Self, Self::Error> {
        let profiles_v2: Result<ProfilesV2, _> = config_v1
            .into_iter()
            .map(|(name, profile_v1)| match ProfileV2::try_from(profile_v1) {
                Ok(profile_v2) => Ok((name.clone(), profile_v2)),
                Err(_) => Err("atlantis"),
            })
            .collect();
        profiles_v2
...
impl Default for SigningKeyPairsV2 {
    fn default() -> SigningKeyPairsV2 {
        SigningKeyPairsV2(HashMap::new())
    }
}
